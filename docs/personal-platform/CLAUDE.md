...
> **Not at repo root — read this file explicitly at the start of any Personal AI Platform
> session.** This repo's root `CLAUDE.md` is a symlink to upstream's own `AGENTS.md`
> (contributor guidance for PRs back to `omnigent-ai/omnigent`) — that file is left untouched
> on purpose, with a one-line pointer added to it directing here. This file governs *our*
> additions (`docs/personal-platform/` and any custom code layered on top), never upstream's
> own project. For contributing changes back upstream, `AGENTS.md` / `CONTRIBUTING.md`
> (upstream's own files, unchanged) are authoritative — DCO sign-off (`git commit -s`),
> branch from `main`, tests before PR.
>
...
> on the primary dev notebook despite an earlier project's `CLAUDE.md` assuming it was —
> an invisible, per-machine dependency. Everything that plugin would normally provide
> (agents, skills, the lifecycle) is committed **into this repo** instead
> (`.claude/agents/`, `.claude/skills/`), so it works identically on any machine that clones
> this fork — the VPS, a notebook, a tablet's browser session, anywhere. See
> `docs/personal-platform/plan.md` (Phase 3) for the full reasoning.
>
...
change, anything touching the upstream sync boundary).
...
| Upstream | `https://github.com/omnigent-ai/omnigent.git` |
| Host | VPS `srv1802750` (`omni-vps`, tailnet `100.116.27.33`), `/opt/omnigent` |
| Working branch | `main` (mirrors upstream's default) |
| Feature branches | `feat/<slug>` for our own additions; keep upstream-facing contribution branches separate and named per `CONTRIBUTING.md` if a change is meant to go back upstream |
...
| `.claude/skills/*/SKILL.md` | Native skill implementations. | normal |
| `CONTRIBUTING.md` (upstream's own, unchanged) | The **upstream** contribution process — DCO, tests, PR template. Authoritative for anything meant to land in `omnigent-ai/omnigent`. | do not edit |
...
- Anything under `.git/config` holding credentials (never commit a token in a remote URL —
  see the incident referenced in §8)
...
3. **Every consequential step leaves a trace** — `/log-activity`; `/log-tracker` at
   milestones. Ad-hoc shortcuts (Hard Rule 3 exemption) still get a `committed` log line via
...
8. **This repo's `upstream` remote is sacred** — never push to it, never rewrite history that
   would complicate future `upstream/main` merges. Our additions live in `docs/personal-
   platform/` and clearly-scoped feature branches specifically so `upstream` syncs stay clean.
...
| System | Identity |
|--------|----------|
| Git (this repo) | `Zuus89/omnigent`, upstream `omnigent-ai/omnigent` — see §8 |
