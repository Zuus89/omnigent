...
# Valid conversation-item types. IDs are prefix-less bare uuid4 hex; the
# item's type lives in its own ``conversation_items.type`` column, not in the
# id. Kept as a validation set so ``generate_item_id`` still rejects unknown
# types (a guard several call sites rely on).
...
def builtin_agent_id(name: str) -> str:
    ...
    Same shape and length as :func:`generate_agent_id` (32 hex chars), but
    ...
    :returns: A deterministic 32-char hex id.
    """
...
def generate_task_id() -> str:
    ...
    :returns: A bare 32-char hex uuid,
        e.g. ``"d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3"``.
    """
    return uuid.uuid4().hex
...
def generate_item_id(item_type: str) -> str:
    ...
    *item_type* is validated against :data:`_ITEM_TYPES` but no longer encoded
    ...
    :returns: A bare 32-char hex uuid, e.g. ``"a1b2c3d4..."``.
